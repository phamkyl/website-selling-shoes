...
//	$pdf->Cell($width_cell[1],10,'Mã hàng',1,0,'C',true);
	...
	$pdf->Cell($width_cell[2],10,$row['name_sp'],1,0,'C',$fill);
	$pdf->Cell($width_cell[3],10,$row['soluongmua'],1,0,'C',$fill);
	$pdf->Cell($width_cell[4],10,number_format($row['selling_price']),1,0,'C',$fill);
	$pdf->Cell($width_cell[5],10,number_format($row['soluongmua']*$row['selling_price']),1,1,'C',$fill);
	$fill = !$fill;
